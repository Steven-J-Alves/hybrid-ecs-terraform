...
variable "container_port" {
  description = "List of container ports to be mapped to target groups"
  type        = list(number)
  default     = []
}

variable "container_name" {
  description = "List of container names associated with each target group"
  type        = list(string)
  default     = []
}

variable "launch_type" {
  description = "Determine the type of infrastructure that host tasks and services"
  default     = "EC2"
}

variable "health_check_grace_period_seconds" {
  description = "Seconds to ignore failing LB health checks on freshly instantiated tasks. Only valid for services with a load balancer."
  type        = number
  default     = null
}