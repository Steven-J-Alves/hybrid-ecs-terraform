...
variable "iam_role_names" {
  ...
  type        = map(string)
  default = {
    devops        = "app2-tf-devops-role"
    ecs           = "app2-tf-ecs-task-execution-role"
    ecs_task_role = "app2-tf-ecs-task-role"
  }
}
...
variable "port_api_app" {
  description = "Port exposed by the app2-api container"
  type        = number
  default     = 4004
}
...
variable "container_name" {
  ...
  type        = map(string)
  default = {
    app2_api        = "container-app2-api"
    app2_worker     = "container-app2-worker"
    app2_scheduler  = "container-app2-scheduler"
    app2_manager    = "container-app2-manager"
    app2_front      = "container-app2-front"
  }
}

variable "port_front_app" {
  description = "Port exposed by the nginx serving app2-front"
  type        = number
  default     = 80
}

variable "app2_host" {
  description = "Public hostname for the app2 frontend (used in Route53 + ALB listener rules)"
  type        = string
  default     = "app2.kriolu-kloud.cv"
}

variable "app2_api_host" {
  description = "Private hostname for the app2 API (used in Route53 + ALB listener rules + API_URL env var)"
  type        = string
